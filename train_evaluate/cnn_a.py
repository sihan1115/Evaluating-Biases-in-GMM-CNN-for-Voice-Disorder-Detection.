# This script trains a CNN model for the vowel 'a' classification task.
...
#RANDOM_SELECTION_SEED = 42  # Controls the seed for random selection
...
def select_seeds_for_vowel_a():
    ...
    if USE_RANDOM_SELECTION:
        ...
        print(f"CNN vowel a [Random Selection] seed combination:")
        ...
        print(f"CNN vowel a [Manual Specification] seed combination:")
        print(f"  TRAIN_VAL_SEED: {train_val_seed}")
        print(f"  CNN_SEED: {cnn_seed}")
...
# Select seeds
TRAIN_VAL_SEED_A, CNN_SEED_A = select_seeds_for_vowel_a()
# Set environment variables to ensure complete determinism (must be set before importing TensorFlow)
os.environ['PYTHONHASHSEED'] = str(CNN_SEED_A)
os.environ['TF_DETERMINISTIC_OPS'] = '1'
os.environ['TF_CUDNN_DETERMINISTIC'] = '1'
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
print(f"Setting environment variable PYTHONHASHSEED = {CNN_SEED_A}")
# ===================================================
...
def set_deterministic_behavior(seed=CNN_SEED_A):
    """
...
def main():
    """CNN vowel 'a' model training"""
    print("=" * 60)
    print("CNN vowel 'a' model training - Random seed version")
    print("=" * 60)
    print(f"Random seed configuration: DEV_TEST={DEV_TEST_SEED}, TRAIN_VAL={TRAIN_VAL_SEED_A}, GMM_INIT={CNN_SEED_A}")
    ...
    # Build and train CNN model for vowel 'a'
    print("\nBuilding and training CNN model for vowel 'a'...")
    set_deterministic_behavior(CNN_SEED_A)
    CNN_a = build_model_cnn2d_original(x_train_a[0].shape, n_classes=n_classes,
                                       k_size=(3, 3), stride_size=(1, 1), pool_size=(2, 3),
                                       act_fn='relu', dropout=0.5, lr=0.00001,
                                       seed=CNN_SEED_A)
    CNN_a.summary()
    with tf.device('/GPU:0' if tf.config.list_physical_devices('GPU') else '/CPU:0'):
        history_a = CNN_a.fit(x_train_a, y_train_a,
    ...
    # Save model
    print("\nSaving vowel 'a' model...")
    model_name_a = generate_model_name('cnn', 'a', DEV_TEST_SEED, TRAIN_VAL_SEED_A, CNN_SEED_A)
    print(f"Model name: {model_name_a}")

    save_model_simple(CNN_a, model_name_a)
    ...
    config_info = {
        ...
        'training_epochs': len(history_a.history['loss'])
    }

    config_name = f"cnn_a_config_dev{DEV_TEST_SEED}_train{TRAIN_VAL_SEED_A}_init{CNN_SEED_A}"
    save_model_config_simple(config_info, config_name)

    print("CNN vowel 'a' model training completed!")
    print(f"Model file: {model_name_a}.h5")
