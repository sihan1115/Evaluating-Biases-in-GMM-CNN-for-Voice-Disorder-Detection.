...
"""
Disparate Impact (DI) Visualization Script
This script creates bar charts with error bars to visualize Disparate Impact (DI) metrics
...
DI measures the ratio of positive prediction rates between unprivileged and privileged groups.
"""
...
METRIC = 'DI'  # Fairness metric to visualize
THRESHOLD = 0.8  # Position of threshold line (common DI threshold is 0.8)
CUSTOM_TITLE = 'DI'  # Custom chart title
...
def create_aod_plot(file_path=None):
    """
    Create Disparate Impact (DI) comparison chart with error bars
    Visualizes DI metrics for GMM and CNN models across different datasets
    ...
    ax.set_ylabel(f'{METRIC} (mean ± std)', fontsize=24, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(scenarios, fontsize=22, fontweight='bold')

    # Set y-axis range (DI values typically range from 0 to 1+)
    ax.set_ylim(0.6, 1.1)
    ...
    save_filename = os.path.join(SAVE_DIR, f'{DIMENSION}_{METRIC}.png')
